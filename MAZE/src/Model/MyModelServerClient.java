...
public class MyModelServerClient {
     ...
     * @param iSearchable
     */
    public void ServerSolvingMaze(ISearchable iSearchable){
        //Initializing server
        Server solveSearchProblemServer = new Server(5401, 1000, new ServerStrategySolveSearchProblem());
        //Starting server
        solveSearchProblemServer.start();
        //Communicating with server
        CommunicateWithServer_SolveSearchProblem(iSearchable);
        //Stopping server
     ...
     * @param iSearchable
     */
    private void CommunicateWithServer_SolveSearchProblem(ISearchable iSearchable) {
        try {
            Client client = new Client(InetAddress.getLocalHost(), 5401, new IClientStrategy() {
                ...
                public void clientStrategy(InputStream inFromServer, OutputStream outToServer) {
                    try {
                        ...
                        ObjectInputStream fromServer = new ObjectInputStream(inFromServer);
                        toServer.flush();
                        toServer.writeObject(((SearchableMaze)iSearchable).getOrigionMaze()); //send maze to server
                        toServer.flush();
